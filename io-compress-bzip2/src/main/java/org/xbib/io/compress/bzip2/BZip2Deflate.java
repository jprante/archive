 ...
 * This class is a rewrite from the CBZip2OutputStream from the Ant program.
...
public class BZip2Deflate implements Bzip2Constants {
    ...
    public int setInput(byte[] b, int offset, int len) throws IOException {
        ...
        if (this.closed || this.inputBuffer.size() > bufferSize) {
            rc = this.inputBuffer.size();
        }
    ...
    public int deflate(byte[] b) {
        ...
        if (!this.wroteHeader) {
            ...
            this.outputBuffer = new ByteArrayInputStream(new byte[]{0x42, 0x5A});
        }

        if (this.outputBuffer != null) {
            try {
                if (this.outputBuffer.available() > 0) {
                    rc = this.outputBuffer.read(b);
                }
            } catch (IOException eIO) {
            }
        }

        if (rc < b.length && this.inputBuffer.size() != 0) {
            if (this.inputBuffer.size() < b.length - rc) {
                byte[] t = this.inputBuffer.toByteArray();
                this.inputBuffer.reset();

                System.arraycopy(t, 0, b, rc, t.length);
                rc += t.length;
            } else {
                byte[] t = this.inputBuffer.toByteArray();
                this.inputBuffer.reset();

                System.arraycopy(t, 0, b, rc, b.length - rc);
                this.outputBuffer = new ByteArrayInputStream(t, b.length - rc, t.length - (b.length - rc));
                rc = b.length;
            }
        }
    ...
    public int deflate(byte[] b, int off, int len) {
        byte[] buff = new byte[len - off];

        int rc = this.deflate(buff);

        if (rc > 0) {
            System.arraycopy(buff, 0, b, off, rc);
        }
     ...
     *
     * @return true is this compressor is finished.
     */
     ...
     *
     * @return true if setInput() should be called before getting more output
     */
    ...
    private void bsFinishedWithStream() throws IOException {
        while (bsLive > 0) {
            ...
            inputBuffer.write(ch); // write 8-bit
    ...
    private void qSort3(int loSt, int hiSt, int dSt) throws IOException {
    ...
    private void mainSort() throws IOException {
        int i, j, ss, sb;
        ...
        // if nothing was written, last is -1, which gives div by zero
        if (last >= 0) {
    ...
    private void doReversibleTransformation() throws IOException {
        int i;
    ...
    private boolean fullGtU(int p1, int p2) {
        int i1 = p1, i2 = p2;
        int k;
        char c1, c2;
        int s1, s2;
